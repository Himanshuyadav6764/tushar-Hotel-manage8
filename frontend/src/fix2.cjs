...
walkDir(dir, function(filePath) {
    if (filePath.endsWith('.js') || filePath.endsWith('.jsx')) {
        ...
        // Match await fetch( and replace with await apiCall(
        newContent = newContent.replace(/await fetch\(/g, 'await apiCall(');
        
        // Match => fetch( and return fetch(
        newContent = newContent.replace(/=> fetch\(/g, '=> apiCall(');
        newContent = newContent.replace(/return fetch\(/g, 'return apiCall(');
